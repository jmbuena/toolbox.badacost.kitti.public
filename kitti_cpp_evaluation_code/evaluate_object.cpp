...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,
                 ...
                 vector<double> &aos, DIFFICULTY difficulty, int first_img, int last_img) {
...
//  int num_images = last_img - first_img + 1;
...
//  system(("mkdir -p " + result_dir).c_str());
  ...
  mail->msg("Loading detections from ...");
  mail->msg("gt_dir=%s", gt_dir.c_str());
  mail->msg("dt_dir=%s", dt_dir.c_str());
  //for (int32_t i=0; i<N_TESTIMAGES; i++) {
  for (int32_t i=first_img; i<=last_img; i++) {
...
//      return false;
  ...
  if(eval_car){
    ...
    if(   !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,first_img,last_img)
       || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,first_img, last_img)
       || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,first_img, last_img)){
  ...
  if(eval_pedestrian){
    ...
    vector<double> precision[3], aos[3];
    if(   !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,first_img,last_img)
       || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,first_img,last_img)
       || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,first_img,last_img)){
  ...
  if(eval_cyclist){
    ...
    vector<double> precision[3], aos[3];
    if(   !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,first_img,last_img)
       || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,first_img,last_img)
       || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,first_img,last_img)){
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // we need 6 arguments!
  if (argc!=6) {
    cout << "Usage: ./eval_kitti_object result_sha gt_dir dt_dir first_img last_img" << endl;
    return 1;
  }
  ...
  string dt_dir = argv[3];
  int first_img; 
  istringstream in(argv[4]); 
  in >> first_img;
  if (!in.eof() || first_img < 0) {
    mail->msg("ERROR: first image index to test is not an integer >0"); 
    return 1;
  }

  int last_img; 
  istringstream in2(argv[5]); 
  in2 >> last_img;
  if (!in2.eof() || last_img < 0) {
    mail->msg("ERROR: last image index to test is not an integer >0"); 
    return 1;
  }
  
  // run evaluation
  if (eval(result_sha,mail,gt_dir,dt_dir,first_img,last_img)) {
